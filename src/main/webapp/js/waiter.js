$(function(){
	...
	obj = {
		...
		add:function(){
			var jq = top.jQuery;    
			var url = 'waiteradd.jsp';
            if (jq("#tabs").tabs('exists', '服务员工增加')){    
                jq("#tabs").tabs('select', '服务员工增加');    
            } else {  
		...
		edit:function(){
			...
			if(rows.length == 1){
	            ...
	            if (jq("#tabs").tabs('exists', '服务员工更新')){    
	                jq("#tabs").tabs('select', '服务员工更新');    
	            } else {  
	            	var content = '<iframe scrolling="auto" frameborder="0"  src="'+url+'" style="width:100%;height:100%;"></iframe>';     
	                jq("#tabs").tabs('add',{    
	                                    title:'服务员工更新',    
	...
	$("#datagrid").datagrid({
		width:700,
		fit:true,
		url:'waiter_paginationQuery.action',
		title:'服务员工信息',
		striped:true,
		rownumbers:true,
		fitColumns:true,
		...
		onDblClickRow:function(rowIndex,rowdata){
			var jq = top.jQuery;    
			var url = 'waiterupdate.jsp?id='+rowdata.id;
            if (jq("#tabs").tabs('exists', '服务员工更新')){    
                jq("#tabs").tabs('select', '服务员工更新');    
            } else {  
            	var content = '<iframe scrolling="auto" frameborder="0"  src="'+url+'" style="width:100%;height:100%;"></iframe>';     
